...
![](https://ws1.sinaimg.cn/large/006tNc79gy1fz6itxyomsj30w80qa14n.jpg)

## 1.2 前端技术架构

![](https://ws1.sinaimg.cn/large/006tNc79gy1fz6ivq3ufwj31qc092td6.jpg)
...
### 2.3.3 模块化编程
[参考文档](https://www.tuicool.com/articles/uuUVBv2)
...
如果我们想运行某个工程，则使用run命令,如果package.json中定义的脚本如下：
...
![](https://ws3.sinaimg.cn/large/006tNc79gy1fz6gv4v0nfj30fe07lwfh.jpg)
...
add(100,200) //返回300
---->
如果方法体中只有一句return语句，可以省略`{}`和`return`关键字
var add = (a,b) => a+b;
add(100,200) //返回300
```
...
```js
export const email = "yuhaowin@126.com"; 
export function showInfo(email) { 
	return email; 
} 
```
...
```js
//导入的时候带花括号
import {email,showInfo} from 'export';
/或者分多次导入
import {email} from 'export';
improt {showInfo} from 'export';
```
...
```js
const email = "yuhaowin@126.com"; 
export default email; 
//其实此处相当于为email变量值"yuhaowin@126.com"起了一个系统默认的变量名default，自然default只能有一个值，所以一个文件内不能有多个export default。 
```

import.js

```js
//导入的时候没有花括号,实际上导出一个default的变量，然后把这个default变量去了一个email的别名；
import email from 'export';
